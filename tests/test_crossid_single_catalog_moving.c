...
main(int argc, char **argv)
{
    ...
    /* move some samples and cross 2 fields */
    move_samples('x', 0.1, fields[0]);
    move_samples('y', 0.1, fields[0]);

    move_samples('x', 0.2, fields[1]);
    move_samples('y', 0.2, fields[1]);

    move_samples('x', 0.1, fields[2]);
    move_samples('y', 0.1, fields[2]);

    //move_samples('x', 0.0, fields[3]);
    //move_samples('y', 0.0, fields[3]);

    move_samples('x', -0.1, fields[4]);
    move_samples('y', -0.1, fields[4]);

    move_samples('x', -0.2, fields[5]);
    move_samples('y', -0.2, fields[5]);

    move_samples('x', -0.1, fields[6]);
    move_samples('y', -0.1, fields[6]);

    //move_samples('x', 0.0, fields[7]);
    //move_samples('y', 0.0, fields[7]);

    move_samples('x', 0.1, fields[8]);
    move_samples('y', 0.1, fields[8]);

    move_samples('x', 0.2, fields[9]);
    move_samples('y', 0.2, fields[9]);


    CrossId_run(&fg, NULL, 2.0*ARCSEC/DEG);
    compare_fields(10, fields);
