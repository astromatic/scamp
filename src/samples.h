...
#define SIGMA_ATMOS		0.054	// Han & Gatewood's 1995 sig_a ...
					// at Mauna Kea for d=10',T=1s [arcsec]
...
//--------------------------------- typedefs --------------------------------
 ...
 typedef enum {
  ASTACC_SIGMA_PIXEL, ASTACC_SIGMA_ARCSEC, ASTACC_TURBULENCE_ARCSEC
 } accuracyenum;
...
//--------------------------- structure definitions -------------------------
 ...
 typedef struct sample {
  ...
  float	wcsproperr[NAXIS];		// World Coords pm uncertainties
  ...
  double	vector[3];		// 3D vector for crossmatching
  float	dprojdred[NAXIS*NAXIS];	// Jacobian of the solution re-projection
 } samplestruct;
...
// ---- MATCHing parameters
  ...
  float	match_sangle;			// Computed shear angle
  float	match_dlng, match_dlat;		// Positional corrections
  float	match_asig, match_sig;		// Angle/scale and position contrasts 
